...
<body class="bg-gray-100 dark:bg-gray-900 min-h-screen">
    <div class="min-h-screen flex items-center justify-center px-4">
        <div class="w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow p-6">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-6">Tambah Transaksi</h1>
            <form action="{{ route('transactions.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input id="amount" name="amount" type="number" step="0.01" min="0" value="{{ old('amount') }}" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200 transition duration-200 shadow-sm {{ $errors->has('amount') ? 'border-red-500 focus:border-red-600 focus:ring-red-600' : '' }}" placeholder="Contoh: 100000">
                    @error('amount')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input id="description" name="description" type="text" value="{{ old('description') }}" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200 transition duration-200 shadow-sm {{ $errors->has('description') ? 'border-red-500 focus:border-red-600 focus:ring-red-600' : '' }}" placeholder="Contoh: Donasi Jumat">
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input id="date" name="date" type="date" value="{{ old('date') }}" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200 transition duration-200 shadow-sm {{ $errors->has('date') ? 'border-red-500 focus:border-red-600 focus:ring-red-600' : '' }}">
                    @error('date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between pt-2">
                    ...
                    <button type="submit" class="inline-flex items-center rounded-md bg-emerald-600 px-4 py-2 text-white hover:bg-emerald-700">Simpan</button>
                </div>
            </form>
        </div>
    </div>
